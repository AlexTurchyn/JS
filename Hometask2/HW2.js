let arr = [{ratingRevievs: "264 отзыва", price: {oldUan: "4 333 грн", newUan: "3 799 грн"}, 
...
// Sort by price
const priceNavigaion = (obj) => {
    if(obj['price'] instanceof Object) return obj['price']['newUan'];
    return obj['price'];
};
const getPrice = (obj) => {
    
    let strPrice = priceNavigaion(obj);

    let reg = /\D/gi;
    strPrice = strPrice.replace(reg,"");

    let numPrice = parseInt(strPrice);
    return numPrice;
}
const sortByPrice = (a,b) => {

    let price1 = getPrice(a);
...
buttonR.textContent = 'Review count ↓';
buttonR.onclick = function(e) {arr.sort(sortByReviewsCount);}
// button.onclick = (sortByReviewsCount) => {mySort(sortByReviewsCount);};
let buttonRR  = document.createElement('button');
buttonRR.textContent = 'Review count ↑';
buttonRR.onclick = function(e) {arr.sort(sortByReviewsCountReverse);}

let buttonP  = document.createElement('button');
buttonP.textContent = 'Price ↓';
buttonP.onclick = function(e) {arr.sort(sortByPrice);}

let buttonPR  = document.createElement('button');
buttonPR.textContent = 'Price ↑';
buttonPR.onclick = function(e) {arr.sort(sortByPriceReverse);}

let buttonConsole  = document.createElement('button');
buttonConsole.textContent = 'See array';
buttonConsole.onclick = function(e) {
    console.clear();
    console.log(arr);
}
...
body.appendChild(buttonRR);
body.appendChild(buttonP);
body.appendChild(buttonPR);
body.appendChild(buttonConsole);
